...
def main():
    ...
    def mDFS(x1, y1, x2, y2, k):
        ...
        for i in range(x1, x2):
            # 得到分割线上方的数值 + 下方继续分割
            U = get(x1, y1, i, y2) + mDFS(i + 1, y1, x2, y2, k - 1)
            # 得到分割线下方的数值 + 上方继续分割
            D = get(i + 1, y1, x2, y2) + mDFS(x1, y1, i, y2, k - 1)
            res = min(res, U, D)
        # 枚举区间 [y1, y2] 中的间隙
        for j in range(y1, y2):
            # 同理，这里是分割为左右两块矩阵
            L = get(x1, y1, x2, j) + mDFS(x1, j + 1, x2, y2, k - 1)
            R = get(x1, j + 1, x2, y2) + mDFS(x1, y1, x2, j, k - 1)
            res = min(res, L, R)
        return res
